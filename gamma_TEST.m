...
hold on
semilogy(et2MJD2000([TOr_prob(~isnan(kk)).t0]),kt*ones(size(kk(~isnan(kk)))),'k--')
hold off
grid on
grid minor
xlim([min(et2MJD2000([TOr_prob(~isnan(kk)).t0])) max(et2MJD2000([TOr_prob(~isnan(kk)).t0]))])
ylabel('$\gamma/\gamma_L$')
...
grid minor
xlim([min(et2MJD2000([TOr_prob(~isnan(kk)).t0])) max(et2MJD2000([TOr_prob(~isnan(kk)).t0]))])
ylabel('$\|\max\left(S_{LEFO,\varepsilon}\right)+\varepsilon\|$')
