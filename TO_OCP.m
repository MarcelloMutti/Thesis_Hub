...
tf=cspice_str2et('2024 OCT 5 22:50 UTC');

fsopt=optimoptions('fsolve','Display','iter-detailed','MaxIterations',1e5,'MaxFunctionEvaluations',1e5);

lltf=fsolve(@(llt) TO_ZFP(llt,t0,SC_param,targ),[l0_g; tf],fsopt);

df=TO_ZFP(lltf,t0,SC_param,targ);

fprintf('pos err %.3e\nvel err %.3e\n',norm(df(1:3))*LU, norm(df(4:6))*LU/TU)

plots(lltf,t0,SC_param,targ);
